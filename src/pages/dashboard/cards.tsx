...
const Cards = observer(({ is_mobile, has_dashboard_strategies }: TCardProps) => {
    ...
    const actions: TCardArray[] = [
        ...
        {
            id: 'free-bots',
            icon: <DerivLightQuickStrategyIcon height='48px' width='48px' />, // Placeholder icon
            content: <Localize i18n_default_text='Free Bots' />,
            callback: () => {
                setActiveTab(DBOT_TABS.BOT_BUILDER);
                // logic to switch to free bots tab if needed
                const tabs_element = document.querySelector('.dc-tabs__list');
                if (tabs_element) {
                    const free_bots_tab = Array.from(tabs_element.querySelectorAll('.dc-tabs__item')).find(
                        tab => tab.textContent === 'Free Bots'
                    );
                    if (free_bots_tab) (free_bots_tab as HTMLElement).click();
                }
            },
        },
    ];
